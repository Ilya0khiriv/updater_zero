...
# === REJECT PIPED EXECUTION (optional but helpful) ===
if [[ "$0" == "/dev/stdin" || "$0" == "bash" || "$0" == "-bash" ]]; then
    log_error "This script must be saved to a file and executed directly."
    log_error "Do NOT run via 'curl ... | bash'."
    exit 1
fi
...
if [[ "$OS" == "macos" ]]; then
    ...
    APP_PATH="$BIN_DIR/vk-uploader"  # ← NO SPACES
...
EOF
    ...
    if [[ "$OS" == "macos" ]]; then
        ...
        PYINST_CMD+=("--name=vk-uploader")  # ← NO SPACE
    fi
...
EOF
        ...
        # Update desktop database (helps in XFCE, KDE, etc.)
